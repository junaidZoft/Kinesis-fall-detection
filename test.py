...
with col2:
    ...
    snapshot_dirs = {
        "FALL": ("fall", "fall-badge"),
        "FIRE": ("fire", "fire-badge"),
        "WEAPON": ("weapon", "weapon-badge")
    }
...
with st.sidebar:
    ...
    status_fall = "🟢 OK" if not Path("fall").glob("*.jpg") else "🔴 Event"
    status_fire = "🟢 OK" if not Path("fire").glob("*.jpg") else "🔴 Event"
    status_weapon = "🟢 OK" if not Path("weapon").glob("*.jpg") else "🔴 Event"
...
with st.sidebar:
    st.markdown("### Detection Status")
    st.markdown("- **Fall**: Saves to `fall/`")
    st.markdown("- **Fire**: Saves to `fire/`")
    st.markdown("- **Weapon**: Saves to `weapon/`")
    st.markdown("---")
...
def start_pipeline(hls_url, model_path, imgsz, conf_thres, use_cuda):
    ...
    # --- ADDED: Print and display CUDA device info ---
    if device.startswith("cuda"):
        try:
            cuda_name = torch.cuda.get_device_name(0)
            st.sidebar.markdown(f"**CUDA device:** {cuda_name}")
            print(f"Using CUDA device: {cuda_name}")
        except Exception as e:
        ...
        try:
            ...
            st.sidebar.markdown(f"- Name: `{props.name}`")
            st.sidebar.markdown(f"- Total Memory: `{props.total_memory // (1024**2)} MB`")
            print(f"CUDA device properties: {props}")
        except Exception as e:
            print(f"Could not get CUDA device properties: {e}")
    else:
        st.sidebar.markdown("**CUDA device:** Not in use (CPU or non-NVIDIA GPU)")
        print("CUDA not in use. Using CPU or non-NVIDIA GPU.")
    # --- END ADDED ---
    with model_lock:
